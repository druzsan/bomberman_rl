"""S1c -- as S1b, but trained against a harder opponent league.
...
The S1b agent scores 5.24 against three `rule_based_agent`s but only 3.16
against three `bfs_expert`s, and its suicide rate doubles from 9 % to 21 % --
it has learned habits that work against an opponent which kills itself half the
time. This config rebalances the Task-4 league towards the strong opponents and
gives it more steps.
